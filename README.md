...
To complete a task, a thread must perform the following steps:
a-> extract the line containing the product from the file
...
a-> add the product to the output file, using the specified format
a-> check if the product specified in the task is the last product found, and notify the order thread that all products have been processed.
